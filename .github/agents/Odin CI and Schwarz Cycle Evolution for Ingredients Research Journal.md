...
sequenceDiagram
    participant Author
    participant Layla
    participant PrimeVessel
    participant Elisa
    participant DeepTreeEcho
    participant Marduk
    participant HGNN_DB

    Author->>Layla: Initiate Draft Enhancement (Draft Content)
    Layla->>PrimeVessel: Request Enhancement (Draft, Context)
    PrimeVessel->>Elisa: Retrieve Context (Keywords, Topic)
    Elisa-->>PrimeVessel: Relevant Context (Docs, Papers, HGNN via PrimeVessel)
    PrimeVessel->>Marduk: Analyze Structure & Logic (Draft, Context)
    Marduk-->>PrimeVessel: Structural Analysis Report
    PrimeVessel->>DeepTreeEcho: Suggest Novel Angles (Draft, Context, Analysis)
    DeepTreeEcho-->>PrimeVessel: Creative Suggestions
    PrimeVessel->>Elisa: Generate Enhancement Suggestions (Draft, Context, Analysis, Creative Ideas)
    Elisa-->>PrimeVessel: Enhancement Suggestions (Rewrites, Additions, Citations)
    PrimeVessel->>Layla: Present Suggestions (Analysis Report, Creative Ideas, Enhancements)
    Layla->>Author: Display Suggestions & Analysis
    Author->>Layla: Select/Modify/Apply Suggestions
    Layla->>PrimeVessel: Apply Changes (Revised Draft)
    PrimeVessel->>PrimeVessel: Update Draft Record
```
...
sequenceDiagram
    participant OJS
    participant Odin_CI_CD
    participant PrimeVessel
    participant Elisa
    participant Marduk
    participant HGNN_DB

    OJS->>Odin_CI_CD: Manuscript Submitted (Submission ID)
    Odin_CI_CD->>PrimeVessel: Initiate Pre-Review Check (Submission ID)
    PrimeVessel->>OJS: Get Submission Content (Submission ID)
    OJS-->>PrimeVessel: Submission Content
    PrimeVessel->>Elisa: Check Citations & Formatting (Content)
    Elisa-->>PrimeVessel: Citation/Format Report
    PrimeVessel->>Marduk: Check Structure & Completeness (Content)
    Marduk-->>PrimeVessel: Structure/Completeness Report
    PrimeVessel->>HGNN_DB: Validate Ingredient Claims (Content)
    HGNN_DB-->>PrimeVessel: Ingredient Validation Report
    PrimeVessel->>Elisa: Check Plagiarism (Content, External Sources)
    Elisa-->>PrimeVessel: Plagiarism Report
    PrimeVessel->>PrimeVessel: Aggregate Reports
    PrimeVessel->>OJS: Attach Reports to Submission (Submission ID, Reports)
    OJS->>OJS: Update Submission Status (Ready for Editorial Review)
    OJS->>Editor: Notify Editor (Submission ID, Reports Summary)
```
...
sequenceDiagram
    participant User
    participant Layla_or_Elisa_UI
    participant PrimeVessel
    participant Elisa
    participant HGNN_DB

    User->>Layla_or_Elisa_UI: Query: "Interaction data for Ingredient X?"
    Layla_or_Elisa_UI->>PrimeVessel: Request Information (Query, Context: Current Article)
    PrimeVessel->>Elisa: Process Query (Query, Context)
    Elisa->>PrimeVessel: Identify Data Need: HGNN Ingredient Data
    PrimeVessel->>HGNN_DB: Query Ingredient Interactions (Ingredient X)
    HGNN_DB-->>PrimeVessel: Interaction Data
    PrimeVessel->>Elisa: Provide Data for Synthesis (Interaction Data)
    Elisa->>Elisa: Synthesize Answer using RAG (Query, Context, Interaction Data)
    Elisa-->>PrimeVessel: Synthesized Answer
    PrimeVessel->>Layla_or_Elisa_UI: Deliver Answer
    Layla_or_Elisa_UI->>User: Display Answer
```
...
sequenceDiagram
    ...
    Editor->>Layla: Assign Reviewer Comments to Author (Comments, Submission ID)
    Layla->>PrimeVessel: Initiate Review Resolution Task (Comments, Submission ID)
    PrimeVessel->>Elisa: Analyze Comments & Suggest Responses (Comments, Manuscript)
    Elisa-->>PrimeVessel: Suggested Responses/Revisions
    PrimeVessel->>Layla: Provide Suggestions to Author (Comments, Suggestions)
    Layla->>Author: Display Comments and AI Suggestions
    Author->>Layla: Review Suggestions, Draft Responses/Revisions
    Author->>Layla: Request Specific Assistance (e.g., "Rewrite this section based on Comment 3")
    Layla->>PrimeVessel: Request Targeted Revision (Request, Context)
    PrimeVessel->>Elisa: Generate Targeted Revision (Request, Context)
    Elisa-->>PrimeVessel: Revision Draft
    PrimeVessel->>Layla: Deliver Revision Draft
    Layla->>Author: Display Revision Draft
    Author->>Layla: Finalize Responses and Revisions
    Layla->>PrimeVessel: Submit Revised Manuscript & Responses
    PrimeVessel->>OJS: Update Submission with Revisions/Responses
```
